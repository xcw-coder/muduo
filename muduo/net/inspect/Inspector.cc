...
#endif
  //这样子做法是为了防止竞态问题
  //如果直接调用start，（当前线程不是loop所属的IO线程，是主线程）那么有可能，当前构造函数
  //还没有返回，HttpServer所在的IO线程可能已经收到了http客户端的请求了（因为这时候HttpServer
  //已经启动了），那么就会调用Inspector::onRequest，而这个时候构造函数还没返回，也就是说
  //对象还没完全构造好
  ...
  else
  {
    ...
    else
    {
      ...
      if (commListI != modules_.end())
      {
        ...
        if (it != commList.end())
        {
          ArgList args(result.begin()+2, result.end());  //传递给回调函数的参数列表
          if (it->second)
          {
            resp->setStatusCode(HttpResponse::k200Ok);
            resp->setStatusMessage("OK");
