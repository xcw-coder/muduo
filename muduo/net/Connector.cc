...
//可以跨线程调用，先将connect_设置为ture，
//然后在loop_所对应的IO线程中，执行startInLoop
...
void Connector::connecting(int sockfd)
{
  ...
  assert(!channel_);
  //在构造函数中并没有初始化channel_，所以在这里让channel_与sockfd关联
...
int Connector::removeAndResetChannel()
{
  ...
  //不能在这里重置channel_，因为当前正在调用Channel::handleEvent
...
void Connector::handleWrite()
{
  ...
  if (state_ == kConnecting)
  {
    ...
    //还需要用getsocopt(sockfd, SOL_SOCKET, SO_ERROR, ...)在确认一下
...
//采用back-off策略重连，即重连时间逐渐延长，0.5s 1s 2s 4s ... 直到30s
