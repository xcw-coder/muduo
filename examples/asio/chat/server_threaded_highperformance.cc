...
class ChatServer : noncopyable
{
 ...
 private:
  void onConnection(const TcpConnectionPtr& conn)
  {
    ...
    if (conn->connected())
    {
      LocalConnections::instance().insert(conn);  //每一个IO线程都有一个ConnectList实例，所以不需要锁
    }
    else
    {
  ...
  {
    LOG_DEBUG << "begin";
    //LocalConnections是thread local变量，所以不需要保护
  ...
  //线程局部单例变量，每个线程都有一个LocalConnections实例
